...
class Generic {
  ...
  extractAssetTrades () {
    ...
    const metadataSection = lines.slice(0, withDateHeader);
    ...
    const trades = records.map((t) => {
      ...
      const each = proceeds.div(qty);
      const feeText = () => format(
        ' with %s fees of %s',
        type === 'BUY' ? 'acquisition' : 'disposal',
        GBP.format(fee)
      );
      const generateDescription = () => format(
        '%s %s at %s each%s', // TODO: non-zero units and bitcoin support
        type === 'BUY' ? 'Bought' : 'Sold',
        asset.formatAmountWithUnit(qty),
        GBP.format(each),
        !fee.eq(ZERO) ? feeText() : ''
      );
      return {
        ...
        totalNetFee: false,
        fee,
        description: t.Description || generateDescription(),
        raw: t,
      };
    });
    return [{
